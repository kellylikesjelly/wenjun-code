...
class AdversarialEnv(multigrid.MultiGridEnv):
  ...
  def __init__(self, 
               n_clutter=50,
               resample_n_clutter=False,    # wenjun: this is True, for the default minigrid env
               size=15, 
               agent_view_size=5, 
               max_steps=250,
               goal_noise=0., 
    ...
    """
    wenjun: some notes for this class
            1. self.wall_locs: a list of tuples, e.g. [(12, 1), (3, 5), (10, 8)], indicating the pos of the blocks
            2. self.wall_mask: a (13,13) array of True/False, indicating the maze map

            Reset_to_level:
              when call reset_to_level(action_traj), action_traj is a list with length=62, however, not all of the actions will be used. 
              there will be a self.adversary_max_steps that limits the num of blocks, and it is dynamic, not always==62. 
              Goal will be placed at step=self.adversary_max_steps-2, and Agent at step=self.adversary_max_steps-1
            Question:
              how is self.adversary_max_steps being linked with an replay_level? i.e., how do we decide what self.adversary_max_steps for a given action_traj?



              
    """
    self.wall_mask_01 = None      # maze map encoded in 0/1 format
  ...
  # '''
  def traj2map(self, action_traj):
    maze_map = np.zeros((self.width-2, self.width-2))
    step_count = 0

    max_steps = int((action_traj[0]/self.adversary_action_dim)*self.n_clutter) + 2
    for idx in range(len(action_traj)):
      if step_count < max_steps:
        step_count += 1

        action_value = action_traj[idx]
        x = int(action_value % (self.width - 2))
        y = int(action_value / (self.width - 2))
        if maze_map[y][x] == 0:
            maze_map[y][x] = 1
        else:
            pass
  ...
  def reset_to_level(self, level):
    """
    wenjun: 
            Reconstruct a level in the replay buffer given an action_traj
            1. line-322: self.resample_n_clutter=True, self.adversary_max_steps=62
               line-336: self.adversary_max_steps=8;
               So, the self.adversary_max_steps must have been changed between line-322 and line-334
    """
    ...
    if isinstance(level, str):
      ...
      # print('reset_to_level: by a sequence of action strings, actions.shape={}, value={}'.format(len(actions), actions))
      
      if self.resample_n_clutter:
        self.adversary_max_steps = len(actions)
      # print('1. self.resample_n_clutter={}, self.adversary_max_steps={}, action.length={}'.format(self.resample_n_clutter, self.adversary_max_steps, len(actions)))
      # self.resample_n_clutter=True, self.adversary_max_steps=62, action.length=62
      # Question: why could adversary_max_steps is alwasy ==62, but the reconstruction have a dynamic max_step value???

      for a in actions:
        obs, _, done, _ = self.step_adversary(a)
        # obs, _, done, _ = self.step_adversary(a, restore=True)
        if done:
    ...
    '''
    # print('reset_to_level, self.wall_locs={}, self.wall_mask={}'.format(self.wall_locs, self.wall_mask))
    maze_map = self.traj2map(actions)
    # print('2. reset_to_level, self.adversary_max_steps={}; actions={}, length={}; self.wall_locs={}, length={}; \n maze_map={}'.format(self.adversary_max_steps, actions, len(actions), self.wall_locs, len(self.wall_locs), maze_map))
    print('2. reset_to_level, action_traj={}, self.wall_locs={}, length={}; \n maze_map={}'.format(actions, self.wall_locs, len(self.wall_locs), maze_map))
    '''
    return obs
  ...
  def _count_walls(self):
    wall_mask = np.array([1 if isinstance(x, minigrid.Wall) else 0 for x in self.grid.grid], dtype=np.bool).reshape(self.height, self.width)[1:-1,1:-1]
    # wall_mask = np.array([1 if isinstance(x, minigrid.Wall) else 5 if isinstance(x, minigrid.Goal) else 0 for x in self.grid.grid], dtype=np.bool).reshape(self.height, self.width)[1:-1,1:-1]
    # wall_mask = np.array([1 if isinstance(x, minigrid.Wall) else 0 for x in self.grid.grid], dtype=np.bool).reshape(self.height, self.width)[1:-1,1:-1]
    self.wall_mask = wall_mask
    
    # self.wall_mask_01 = wall_mask.astype(np.int32)
    # print('wenjun debug: wall_mask={}'.format(self.wall_mask_01))
  ...
  # def step_adversary(self, loc, restore=False):
    ...
    # if restore:
      # print('3. self.adversary_max_steps={}, self.resample_n_clutter={}, self.n_clutter_sampled={}'.format(self.adversary_max_steps, self.resample_n_clutter, self.n_clutter_sampled))
      # when reconstruct step==0: self.resample_n_clutter = True, self.n_clutter_sampled = False. --> e.g. self.adversary_max_steps=62
      # when reconstruct step >0: self.resample_n_clutter = True, self.n_clutter_sampled = True.  --> e.g. self.adversary_max_steps=20, 56, etc.

    # Resample block count if necessary, based on first loc
    if self.resample_n_clutter and not self.n_clutter_sampled:
      n_clutter = int((loc/self.adversary_action_dim)*self.n_clutter)    # wenjun: num_block is defined by the first value in the action_traj 
      self.adversary_max_steps = n_clutter + 2
      self.n_clutter_sampled = True
    ...
    if self.adversary_step_count < self.adversary_max_steps:
      ...
      # Place wall
        ...
        if self.grid.get(x, y) is None:
          ...
          self.wall_locs.append((x-1, y-1))
          # if restore:
            # print('4. restore level: step-[{}/{}], action_value={}, put a wall at loc=({}, {})'.format(self.adversary_step_count, self.adversary_max_steps, loc, x, y))
        # elif restore:
            # print('4. restore level: step-[{}/{}], action_value={}, loc=({}, {}) has a wall ready'.format(self.adversary_step_count, self.adversary_max_steps, loc, x, y))
