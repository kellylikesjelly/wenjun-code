...
class BipedalWalkerAdversarialEnv(BipedalWalkerCustom):
    ...
    def encoding(self):
        ...
        # print('encoding: level_params_vec={}, self.level_seed={}'.format(self.level_params_vec, self.level_seed))
    ...
    def mutate_level(self, num_edits=1):
        if num_edits > 0:
            ...
            for a,d in zip(edit_actions, edit_dirs):
                ...
                self.level_params_vec[a-1] = np.clip(self.level_params_vec[a-1]+mutation, *PARAM_RANGES_FULL[a])
    ...
    def step_adversary(self, action):
        ...
        # get unnormalized value from the action, mapping action=[-1, +1] to corresponding params range
        value = ((action + 1)/2) * (val_range[1]-val_range[0]) + val_range[0]       
