...
class RolloutStorage(object):
    def __init__(self, 
        ...
        self.level_diversity = torch.zeros(num_steps, num_processes, 1, dtype=torch.float)
    ...
    def to(self, device):
        ...
        self.level_diversity = self.level_diversity.to(device)
    ...
    def insert(self, obs, recurrent_hidden_states, actions, action_log_probs, action_log_dist, value_preds, rewards, masks, bad_masks, 
                     level_seeds=None, cliffhanger_masks=None):
    ...
    def insert_divergence(self, level_divergence):
        # wenjun: update level_divergence after student policy rollout, so we need to copy level_div to all previous steps
        self.level_diversity[:].copy_(level_divergence)
        # print('insert_div: level_div[0]={}, level_div[100]={}, level_div[200]={}'.format(self.level_diversity[0], self.level_diversity[100], self.level_diversity[200]))
    ...
    def compute_gae_returns(self, 
        ...
        for step in reversed(range(self.rewards.size(0))):
            delta = self.rewards[step] + gamma*value_preds[step + 1]*self.masks[step + 1] - value_preds[step]
    ...
    def compute_discounted_returns(self,
        ...
        for step in reversed(range(self.rewards.size(0))):
            returns_buffer[step] = returns_buffer[step + 1] * gamma * self.masks[step + 1] + self.rewards[step]
