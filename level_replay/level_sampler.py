...
class LevelSampler():
    """ wenjun:
    adversarail_runner updates the seed scores with two functions:
    1. update_with_rollouts()
    2. after_update()
    ...
    def __init__(
        ...
        obs_space, 
        action_space, 
        num_actors=1,               # wenjun: num_actors=args.num_processes
        strategy='random', 
        max_score_coef=0.0,
        ...
        sample_full_distribution=False,         # wenjun: default=True
        seed_buffer_size=0, 
        ...
        # wenjun: Diversity Metric
        # self.INCLUDE_DIVERSITY = True
        # self.INCLUDE_DIVERSITY = False
        self.seed_diversity = np.array([1.]*N, dtype=np.float)      # init a high div, so that every seed can be sampled
        self.diversity_transform = 'rank'
        self.diversity_temperature = 4.0
        
        self.diversity_coef_support = 1.0/3

        self.staleness_coef_support = 1.0/3
        self.staleness_coef_replay = 1.0/2


        self.combine_div_regret = True  # combine div and regret
        self.use_solely_div = True      # use solely divergence to pick levels into the level buffer


        # log diversity metric
        self.regret_sum = 0

        # init diversity value
        # self.diversity_init = torch.zeros((32, ))
    ...
    def update_seed_score(self, actor_index, seed, score, max_score, diversity, num_steps):
        if self.sample_full_distribution and seed in self.staging_seed_set:
            ...
            # print('1. _partial_update_seed_score_buffer..., self.sample_full_distribution={}, self.staging_seed_set={}'.format(self.sample_full_distribution, self.staging_seed_set))
            # wenjun: self.sample_full_distribution=True, self.staging_seed_set is a decreasing list, gradually remove seed one by one from the list
            # i.e., {45, 46, 47, 48} -> {46, 47, 48} -> {47, 48} -> {48} -> set()
        else:
            score, seed_idx = self._partial_update_seed_score(actor_index, seed, score, max_score, diversity, num_steps, done=True)
            # print('2. _partial_update_seed_score...')

        return score, seed_idx

    def _partial_update_seed_score(self, actor_index, seed, score, max_score, diversity, num_steps, done=False):
        """
        wenjun: 
        this function update self.partial_seed_scores[][]
        """
        ...
        if done:
            ...
            # wenjun: update level diversity here
            self.seed_diversity[seed_idx] = diversity
            # print('_partial_update_seed_score: seed_idx-{}, diversity={}'.format(seed_idx, diversity))
    ...
    def _partial_update_seed_score_buffer(self, actor_index, seed, score, diversity, num_steps, done=False):
        """
        wenjun: 
        this function will remove the seed with minimum support.
        i.e. it maintains the buffer of seed_scores, seed_staleness, etc. while the above function only updates seed_scores, not maintaining the buffer of it
        """
        seed_idx = -1
        ...
        if done:
            ...
            if self.seed_scores[seed_idx] <= merged_score or self.unseen_seed_weights[seed_idx] > 0:
                ...
                # wenjun: update level diversity here
                self.seed_diversity[seed_idx] = diversity       
                # print('_partial_update_seed_score_buffer: seed_idx-{}, diversity={}'.format(seed_idx, diversity))
        ...
        if self._proportion_filled < 1.0:
            ...
            if self.seed_buffer_priority == 'replay_support':
                if self.combine_div_regret:
                    ...
                    return self.sample_weights().argmin()                 # original: support = {score, stale, }
            else:
    ...
    def _update_with_rollouts(self, rollouts, score_function):
        ...
        level_diversity = rollouts.level_diversity
        # print('_update_with_rollouts: level_seeds.shape={}'.format(level_seeds.shape))
        # wenjun: level_seeds=(1,2,3,...,16), shape=(2048,16,1). For each step, store <s,a,r,s'> with corresponding seeds
        ...
        for actor_index in range(num_actors):
            ...
            for t in done_steps:
                ...
                # wenjun: get diversity from rollouts, i.e. storage
                seed_t = level_seeds[start_t,actor_index].item()
                diversity_t = level_diversity[start_t,actor_index].item()
                # print('actor_index={}, seed_t={}'.format(actor_index, seed_t))
                # wenjun: actor_index is 0~15. seed_t is the level_seed for each timestep & actor. 
                # There could be multiple seed_t being printed, because we collect multiple traj until timestep=2048
            ...
            if start_t < total_steps:
                ...
                if self.sample_full_distribution and seed_t in self.staging_seed_set:
                    self._partial_update_seed_score_buffer(actor_index, seed_t, score, diversity_t, num_steps)
                else:
                    self._partial_update_seed_score(actor_index, seed_t, score, max_score, diversity_t, num_steps)
        ...
        for actor_index in range(self.partial_seed_scores.shape[0]):
            for seed_idx in range(self.partial_seed_scores.shape[1]):
                if self.partial_seed_scores[actor_index][seed_idx] != 0:
                    # self.update_seed_score(actor_index, self.seeds[seed_idx], 0, float('-inf'), 0)         # original  
                    self.update_seed_score(actor_index, self.seeds[seed_idx], 0, float('-inf'), 0, 0)        # wenjun, diversity=0, init a low value
                    # self.update_seed_score(actor_index, seed, score, max_score, diversity, num_steps)
            ...
            for actor_index in range(self.num_actors):
                ...
                for seed in actor_staging_seeds:
                    if self.partial_seed_scores_buffer[actor_index][seed] > 0:
                        # self.update_seed_score(actor_index, seed, 0, float('-inf'), 0)            # original
                        self.update_seed_score(actor_index, seed, 0, float('-inf'), 0, 0)           # wenjun, diversity=0, init a low value
    ...
    # wenjun: only replay the env with highest regret
    '''
    def sample_highest_prob_level(self, update_staleness=True):
        ...
        # seed_idx = np.argmax(self.seed_scores)        # 1. depends on {regret}
        seed_idx = np.argmax(sample_weights)            # 2. depends on {regret, stale}
        
        self.replay_probs = sample_weights
    ...
    def sample_weights_w_diversity(self):
        """
        wenjun: replay buffer selection criterion with diveristy
                Note: we only include diversity when picking levels into the buffer. When we compute the replay probs, we don't consider diversity    
        """
        ...
        self.score_probs = weights
        
        # wenjun: compute diversity probs
        diversity_weights = 0
        staleness_weights = 0
        if self.diversity_coef_support > 0 and self.staleness_coef > 0:
            # diversity 
            ...
            self.diversity_probs = diversity_weights
            ...
            self.stale_probs = staleness_weights

            # support = {score, diversity, stale}
            weights = (1 - self.diversity_coef_support - self.staleness_coef_support)*weights + self.diversity_coef_support*diversity_weights + self.staleness_coef_support*staleness_weights
        return weights

    def sample_weights_solely_diversity(self):
        """
        wenjun: replay buffer selection criterion solely with diveristy
                Note: we only include diversity when picking levels into the buffer. When we compute the replay probs, we don't consider diversity    
        """
        # wenjun: compute diversity probs
        diversity_weights = 0
    ...
    def sample_weights(self):
        ...
        if self.staleness_coef > 0:
            ...
            self.stale_probs = staleness_weights
            weights = (1 - self.staleness_coef_replay)*weights + self.staleness_coef_replay*staleness_weights
